...
FILENAME = "demo.txt"  # expected 66
FILENAME = "demo1.txt"  # expected 31
FILENAME = "input.txt"
...
class Node:
    def __init__(
        ...
        self._value: int = NONE_INT

    def __repr__(self) -> str:
        result = []
        if self.parent is not None:
            result.append(f"node{self.name} =>")
        result.append(f"Node({self.name}) => (")
        for child in self.children:
            result.append(f"Node{child.name}")
        result.append(f"){self.metadata}")
        return " ".join(result)

    def value(self) -> int:
        if self._value != NONE_INT:
            return self._value
        if len(self.children) == 0:
            return sum(self.metadata)
        result = 0
        for num in self.metadata:
            if 0 <= num - 1 < len(self.children):
                result += self.children[num - 1].value()
        self._value = result
        return result
