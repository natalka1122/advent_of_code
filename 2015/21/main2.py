...
FILENAME, MY_HIT_POINTS, MY_DAMAGE, MY_ARMOR = "demo.txt", 8, 5, 5  # expected None
    ...
    for armor in ARMOR:
        for weapon in WEAPONS:
            for ring1, ring2 in itertools.combinations(RINGS.keys(), 2):
                ...
                if most_gold is not None and current_gold <= most_gold:
                    continue
                if not myself.fight(
                    enemy,
                    [WEAPONS[weapon], ARMOR[armor], RINGS[ring1], RINGS[ring2]],
                ):
                    most_gold = current_gold
                    print(most_gold, armor, weapon, ring1, ring2)
    print(most_gold)
